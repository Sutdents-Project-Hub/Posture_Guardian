...
目前已完成 Email／密碼登入與註冊、全頁面登入守門、AI-first 首頁、房間自適應／固定正面／固定側面、半身／全身品質引導、10 秒校準、5 秒滾動中位數、8 秒有效偏移事件、3 秒有效回正、分級提醒、摘要、週報、合格工作階段改善圖、VPS／PostgreSQL／量界智算證據鏈、提醒感受、隱私設定，以及可持久化的跟隨系統／淺色／深色外觀。未登入時只顯示登入／註冊；登入後首頁可選擇不需相機、API 或 MediaPipe 的展示模式，供決賽備援，且不建立或污染正式工作階段歷史與週報。
...
- 主入口「房間自適應」使用 `auto` 並預設背鏡頭；另保留固定側面與固定正面。`auto` 只在正面／側面辨識穩定時分析。
- API 回傳解析視角、`upper_body/full_body`、距離、截斷、多人與品質資訊；畫面同步顯示目前半身／全身、距離與調整原因。
- 房間模式只支援固定鏡頭內的一位主要使用者。人物過小、多人、必要節點截斷或視角不穩定時拒絕影格，不建立低品質判定。
- 未登入時只可進入登入／註冊；所有產品畫面（含展示模式）完成驗證後才能開啟。
- 依畫面引導完成 10 秒校準，有效骨架影格需達 80%；auto 校準只納入穩定同一解析視角且具有相容 metrics 的影格。
- 指標先取最近 5 秒有效值的中位數；有效偏移累積 8 秒後進入事件，無效影格不會偷算時間；回到門檻內連續 3 秒才解除。
- 結束後顯示良好坐姿率、提醒次數、平均分數與建議。
- 展示模式固定使用本地摘要，不建立、上傳或完成 API 工作階段；登入後才可選擇它，只有正式觀察且至少 10 分鐘有效資料的工作階段會進入改善趨勢與階段評估。
...
- 歷史頁透過 `GET /api/v1/reports/weekly?timezone=Asia/Taipei` 顯示本週 `good/attention/poor/invalid` 狀態、凌晨／上午／下午／晚上四時段與提醒次數；缺少資料的區段不以 0% 冒充良好。建議重用最近一次已保存的安全 session insight；沒有可用 insight 時明確顯示 fallback，重新整理不重複呼叫外部 AI。
- 設定頁的安靜模式只保留畫面提示；關閉安靜模式後，進階／加強階段才會使用可關閉的手機震動。系統沒有聲音推播或物理按壓硬體。
...
- MediaPipe Full 的房間支援距離、人物最小尺寸、auto 視角、半身／全身切換與全身新指標仍需在決賽裝置以知情同意真人完成驗證。
- 「房間自適應」不代表任意位置或跨房間追蹤；多人、過遠、出框、遮擋與背面／斜角不明會停止分析。
