...
目前已完成 AI-first 首頁、正面／側面設定、10 秒校準、5 秒滾動中位數、8 秒有效偏移事件、3 秒有效回正、分級提醒、摘要、合格工作階段改善圖、AI／Azure 證據鏈、提醒感受、隱私設定，以及可持久化的跟隨系統／淺色／深色外觀。雙主題共用方角編輯式視覺系統，以暖象牙、陶土橘、蜂蜜金與墨藍呈現；首頁另有不需相機的展示模式，供決賽備援。
...
- 只有至少 10 分鐘有效資料的工作階段會進入改善趨勢與階段評估；短展示會明確標為資料不足。
- 「AI 洞察」會分開呈現真實 Foundry 成功、已設定待呼叫與規則式 fallback，並顯示模型部署名稱、prompt version 與去識別輸入摘要。
