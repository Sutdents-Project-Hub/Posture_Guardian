...
class PostureSession(Base):
    ...
    coverage_mode: Mapped[str] = mapped_column(String(16), default="upper_body")
    room_mode: Mapped[bool] = mapped_column(Boolean, default=False)
    ...
    attention_seconds: Mapped[float] = mapped_column(Float, default=0)
    poor_seconds: Mapped[float] = mapped_column(Float, default=0)
...
class PostureSample(Base):
    ...
    __table_args__ = (
        Index("ix_posture_samples_session_captured_at", "session_id", "captured_at"),
    )
    ...
    reminder_triggered: Mapped[bool] = mapped_column(Boolean, default=False)
