...
| Microsoft Foundry Models | 由摘要產生繁中改善建議，提供 Azure PoC | Responses API provider 已實作；資源／額度未確認 | provider=`fallback` 使用固定規則建議 |
...
2. 目前以 Responses API 產生 70 字內中性繁中建議；若之後需要更嚴格展示證據，再改成結構化 schema。
