...
| API | `backend` | Uvicorn port 8000、PostgreSQL 連線、`/live` liveness、503 readiness、multipart 影像分析 | Coolify CPU／RAM、cold start、proxy timeout 與 rate limit |
...
4. `/live` 只檢查 process；`/health` 檢查資料庫與姿態模型並在退化時回傳 503。Foundry 失效時以 8 秒 timeout 轉為 fallback，不阻擋核心姿勢規則。
5. 測試帳號與資料去識別；沒有 raw frame、secret 或 debug log。
6. 使用決賽設備完成 HTTPS、相機權限、CORS 與 API latency 驗收。
7. 匿名 API 只作受控 demo；若直接暴露公網，先以 Coolify／反向代理加存取控制與 rate limit。正式多使用者版需另做身份與資料 ownership。
