...
- SQLAlchemy 已建立 `posture_sessions` 與 `posture_samples` schema；本機預設 SQLite，production URL 可切 PostgreSQL。
- API 已能建立、追加衍生 sample、完成、查詢及刪除匿名工作階段。
- 尚未加入 migration／seed；prototype 啟動時以 `create_all` 建表。
...
4. 資料庫只寫入聚合指標與工作階段摘要。
5. Microsoft Foundry 只接收去識別摘要，例如時段、良好比例、常見偏移與介入階段。
...
- migration 工具與 backup／restore 流程。
